...
const libraryGames = JSON.parse(fs.readFileSync(path.join(repoRoot, "data", "library-games.json"), "utf8"));

test("library games define a non-empty collection with unique ids", () => {
  assert.ok(Array.isArray(libraryGames));
  assert.ok(libraryGames.length > 0);

  const ids = libraryGames.map((game) => game.id);
  assert.equal(new Set(ids).size, ids.length);
});

test("library games include the required catalog fields", () => {
...
test("library games use local cover assets that exist inside the project", () => {
  for (const game of libraryGames) {
    assert.ok(!/^https?:\/\//i.test(game.image), `Library image should be local for ${game.id}`);
...
test("library games keep plausible year and price ranges", () => {
  for (const game of libraryGames) {
