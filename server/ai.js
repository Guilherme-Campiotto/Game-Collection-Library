...
function normalizeIdentifiedGame(game) {
...
async function identifyGameFromPhoto(repoRoot, { dataUrl }) {
  ...
  const prompt = [
    ...
    "Analise a foto enviada, identifique o jogo e, se possível, a plataforma exata.",
    "Use busca web para estimar o preço médio atual no Brasil, preferindo Mercado Livre quando houver resultados relevantes.",
    ...
    "}",
    "Se algum dado não puder ser confirmado, use uma estimativa conservadora e explique em notes."
  ].join("\n");
  ...
  const identified = normalizeIdentifiedGame(parseJsonFromText(extractResponseText(payload)));
  identified.image = saveUploadedCover(repoRoot, {
    fileName: `${identified.platform}-${identified.title}`,
    dataUrl
  });
